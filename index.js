...
⣿⣿⡿⡅⠄⠄⢀⡰⠂⣼⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿`);
    ...
    case 4:
      tipofil = parseInt(
        prompt(`Tipo de filtrado\n1-Nombre\n2-precio\n3.Salir`)
      );
      switch (tipofil) {
        case 1:
          let nombrefil = prompt("Palabra que quiere filtrar:");
          const resultado = Componentes.filter((componente) =>
            componente.nombre.includes(nombrefil)
          );
          alert(resultado);
          break;
        case 2:
          ...
          switch (maymen) {
            case 1:
              ...
              const resulmay = Componentes.filter(
                (componente) => componente.precio >= may
              );
              console.log(resulmay);
              break;
            case 2:
              let men;
              may = parseInt(prompt(`El precio es menor a:`));
              const resulmen = Componentes.filter(
                (componente) => componente.precio <= men
              );
